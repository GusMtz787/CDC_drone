...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		//Linear dynamics
		force = (RotationMatrix(attitude_position).inverse() * (quad_mass * gravity * e3)) - (thrust * e3) - dist; 
		//force = (RotationMatrix(attitude_position).inverse() * (quad_mass * gravity * e3)) - (thrust * e3); 
